...
RUSTC=/opt/rust-hikari/bin/rustc \
	...
	/opt/rust-hikari/bin/cargo build --release --target aarch64-apple-ios --package activator --features trial || exit 1
...
dpkg-deb -Zxz -b .theos/_ target/me.aspenuwu.zinnia.trial_"$VERSION"_iphoneos-arm64.deb || exit 1
