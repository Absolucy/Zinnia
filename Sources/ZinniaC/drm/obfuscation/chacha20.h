...
static inline __attribute__((always_inline)) uint32_t rotl32(uint32_t x, int n) {
	return (x << n) | (x >> (32 - n));
}

static inline __attribute__((always_inline)) uint32_t pack4(const uint8_t* a) {
	uint32_t res = 0;
...
static inline __attribute__((always_inline)) void unpack4(uint32_t src, uint8_t* dst) {
	dst[0] = (src >> 0 * 8) & 0xff;
	dst[1] = (src >> 1 * 8) & 0xff;
	dst[2] = (src >> 2 * 8) & 0xff;
	dst[3] = (src >> 3 * 8) & 0xff;
...
static inline __attribute__((always_inline)) void chacha20_init_block(struct chacha20_context* ctx, uint8_t key[],
																	  uint8_t nonce[]) {
...
static inline __attribute__((always_inline)) void chacha20_block_set_counter(struct chacha20_context* ctx,
																			 uint64_t counter) {
	ctx->state[12] = (uint32_t)counter;
	ctx->state[13] = pack4(ctx->nonce + 0 * 4) + (uint32_t)(counter >> 32);
}

static inline __attribute__((always_inline)) void chacha20_block_next(struct chacha20_context* ctx) {
