...
void blake3_hash4_neon(const uint8_t* const* inputs, size_t blocks, const uint32_t key[8], uint64_t counter,
	...
	for (size_t block = 0; block < blocks; block++) {
		...
		uint32x4_t v[16] = {
			...
			h_vecs[6],		 h_vecs[7],		   set1_128(IV[0]), set1_128(IV[1]), set1_128(IV[2]), set1_128(IV[3]),
...
INLINE void hash_one_neon(const uint8_t* input, size_t blocks, const uint32_t key[8], uint64_t counter, uint8_t flags,
